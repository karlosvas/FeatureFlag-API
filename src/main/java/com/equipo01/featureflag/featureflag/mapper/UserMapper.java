 ...
 *
 * <p>This interface provides bi-directional mapping functionality between User domain entities and
 * their corresponding Data Transfer Objects (DTOs). It handles the conversion logic needed to
 * maintain separation between the data layer and the presentation/API layer for user-related
 * operations.
 *
 ...
 *
 * <p>Key mapping capabilities: - Bi-directional single entity conversions (User ↔ UserDTO) -
 * Collection mappings for bulk operations - Request DTO to entity conversion with default values -
 * Automatic field mapping based on name matching
 *
 * <p>Default value assignments: - New users are assigned "USER" role by default - New users are set
 * as active (true) by default
 */
