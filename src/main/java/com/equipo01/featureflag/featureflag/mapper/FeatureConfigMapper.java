 ...
 * 
 * This interface provides mapping functionality between FeatureConfig domain
 * entities and their corresponding Data Transfer Objects (DTOs) for both request
 * and response scenarios. It handles the conversion logic needed to maintain
 * separation between the data layer and the presentation/API layer for feature
 ...
 * - Request DTO to entity conversion with feature relationship management
 * - Entity to response DTO conversion with feature ID extraction
 * - Collection mappings for bulk operations
 * - Proper handling of nested feature relationships
 * 
 * Special mapping considerations:
 * - Feature relationship is ignored during entity creation (managed separately)
 * - Feature ID is extracted from the nested feature entity for response DTOs
